...
{
	document.getElementById("name1").innerHTML = document.getElementById('emp').value;
	...
	// console.log(",,,,,,,,,,,,,,,,,,,,,,,,,",color.r,",,,,,,,,,,,,",color.g,",,,,,,,,,,",color.b);
			...
			console.log('.......',rrr,this.dat,this.bbb);
		if(rrr){
	...
	if(color.r == 255 && color.g < 127 && color.b < 127) {
		if(this.bbb){
			...
			Qcount=Math.floor((Math.random() * 174) + 1);
			getquestion(Qcount);
			$('body').append('#intro2');
			$('#intro2').css('display', 'inline-block');
			this.movement.z += this.boost * dt*5;
			this.bbb = false;
			this.speed = 0;
			this.boost = 0;
			this.active = false;
...
// $('#submit').hide();
//
//  if(answers[anscount]==get_radio_value()) {
// 	 				console.log("correct");
// 	 				$('#explanation').hide();
// 					$('#ANID').text('Right').css("color","green");
// 					$('body').append('#explanation').hide();
// 					$('#explanation').fadeOut();
// 				$('#rightexptable').css('display', 'inline-block');
// 				// correctans ++;
// 				// $('#score').html(correctans * 100);
// 				}
// 				else
// 				{
// 					console.log("wrong");
// 				$('#rightexptable').hide();
// 				$('#AID').text('Wrong').css("color","red");
//
// 				$('body').append('#explanation');
// 				$('#exptable').css('display', 'inline-block');
// 				$("#EID").text(explanation[anscount]).css("color","green");
// 				// health -= 10;
// 				// $('#health').html(health);
// 				}
// $('#explanation').hide();
...
function get_radio_value() {
			...
			function disappear(){
				console.log('before',this.rrr);
				this.rrr = true;
					console.log('aft',this.rrr);
			 $('#intro2').fadeOut();
			 $('#explanation').fadeOut();
			  $('#exptable').css('display', 'none');
			  $('#rightexptable').css('display', 'none');
			 QFlag = false;
			//  controls.movementSpeed = MOVESPEED;
			// controls.lookSpeed = LOOKSPEED;
			// controls.lookVertical = false; // Resume the camera
			// controls.noFly = true;
			 }
